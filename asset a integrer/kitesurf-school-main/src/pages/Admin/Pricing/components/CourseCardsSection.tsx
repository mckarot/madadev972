// src/pages/Admin/Pricing/components/CourseCardsSection.tsx
// Section d'affichage et gestion des CourseCards dans la page Admin Pricing

import { useState } from 'react';
import type { CourseCard } from '../../../../types';
import { CourseCardModal } from './CourseCardModal';
import { motion } from 'framer-motion';
import { Edit2, Trash2, ToggleLeft, ToggleRight, Star, Users, User, UsersRound, Highlighter, Award, Wind, Clock, Calendar, Check, Heart, Zap, Target, Trophy, Medal, Flame, Droplet, Sun, Moon, Cloud, CloudRain } from 'lucide-react';
...
export function CourseCardsSection({
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer cette CourseCard ?')) {
  ...
  if (isLoading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 font-medium">Chargement des CourseCards...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">CourseCards</h3>
          <p className="text-gray-600 mt-1">Gérez les cartes de cours (Collectif, Particulier, Duo)</p>
        </div>
        <motion.button
      ...
      {cards.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-3xl shadow-xl border-2 border-purple-100">
          ...
          <p className="text-2xl font-bold text-gray-900 mb-3">Aucune CourseCard</p>
          <p className="text-gray-500 mb-6">Créez votre première carte de cours</p>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={handleCreate}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-baseline mb-4">
                  ...
                  <span className="text-gray-600 ml-2 text-sm">/séance</span>
                </div>
                ...
                <div className="space-y-2 mb-4 text-sm">
                  <div className="flex items-center text-gray-700">
                    ...
                    <span>Max {card.maxStudents} élèves</span>
                  </div>
                  <div className="flex items-center text-gray-700">
